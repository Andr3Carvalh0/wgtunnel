...
internal data class InterfaceProxy(
	...
	fun toWgInterface(): Interface = Interface.Builder().apply {
	...
	fun toAmneziaCompatibilityConfig(): InterfaceProxy = copy(
		junkPacketCount = "4",
	...
	fun resetAmneziaProperties(): InterfaceProxy = copy(
		junkPacketCount = "",
		junkPacketMinSize = "",
		junkPacketMaxSize = "",
		initPacketJunkSize = "",
	...
	fun isAmneziaCompatibilityModeSet(): Boolean = junkPacketCount.toIntOrNull() in 3..<5 &&
	...
	fun toAmInterface(): org.amnezia.awg.config.Interface = org.amnezia.awg.config.Interface.Builder().apply {
		parseAddresses(addresses)
		parsePrivateKey(privateKey)
		if (dnsServers.isNotBlank()) parseDnsServers(dnsServers)
		if (mtu.isNotBlank()) parseMtu(mtu)
	...
	companion object {
		fun from(i: Interface): InterfaceProxy = InterfaceProxy(
		...
		fun from(i: org.amnezia.awg.config.Interface): InterfaceProxy = InterfaceProxy(
			publicKey = i.keyPair.publicKey.toBase64().trim(),
			privateKey = i.keyPair.privateKey.toBase64().trim(),
			addresses = i.addresses.joinToString(", ").trim(),
